...
const Home = () => {
  ...
  return (
    <SafeAreaView style={[globalStyle.flex, globalStyle.backgroundColor]}>
      <Search placeHolder={'Search..'} onSearchComplete={setWeather} />
      {weather && (
        <View>
          <View style={style.cityNameContainer}>
            <Text style={style.cityNameText}>{weather.location?.name},</Text>
            <Text style={style.countryNameText}>{weather.location?.country}</Text>
          </View>
          ...
          <View style={style.todayDegreeContainer}>
            <Text style={style.degreeText}>{weather.current?.temp_c + "°"}</Text>
            <Text style={style.descText}>{weather.current?.condition?.text}</Text>
          </View>
          <View style={style.iconContainer}>
            <View style={style.iconTextContainer}>
              ...
              />
              <Text style={style.iconTextMargin}>{weather?.current?.wind_kph}</Text>
            </View>
            <View style={style.iconTextContainer}>
              <Image
                style={style.icon}
                source={require('../../assets/images/wind.png')}
              />
              <Text style={style.iconTextMargin}>{"%" + weather?.current?.humidity}</Text>
            </View>
            <View style={style.iconTextContainer}>
              <Image
                style={style.icon}
                source={require('../../assets/images/wind.png')}
              />
              <Text style={style.iconTextMargin}>{weather?.forecast?.forecastday?.[0]?.astro?.sunrise}</Text>
            </View>
          </View>
          ...
          >
            <View style={style.forecastContainer}>
              ...
              <Text style={style.forecastDailyDegree}>{weather?.forecast?.forecastday?.[0]?.day?.avgtemp_c}</Text>
            </View>
            ...
            <View style={style.forecastContainer}>
              ...
              <Text style={style.forecastDailyDegree}>{weather?.forecast?.forecastday?.[1]?.day?.avgtemp_c}</Text>
            </View>

            <View style={style.forecastContainer}>
              <Image
                source={{
                  uri: "https:" + weather?.forecast?.forecastday?.[2]?.day?.condition?.icon
                }}
                style={style.scrollImage}
              />
              <Text style={style.forecastDailyDegree}>Wednesday</Text>
              <Text style={style.forecastDailyDegree}>{weather?.forecast?.forecastday?.[2]?.day?.avgtemp_c}</Text>
            </View>
